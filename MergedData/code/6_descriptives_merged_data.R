...
id.vars <-
    c("qid", "zip", "bene_dod", "hmoind", "fips",
      "fips_no_interp", "latitude", "longitude",
      "zcta", "zcta_no_interp")
cat.vars <-
    c("sex", "race", "dodflag", "dual", "dead", "cluster_cat",
      "entry_age_break", "statecode")
num.vars <-
    c("age", "year", "pm25_nn", "pm25_ensemble", "pm25_no_interp",
      "ozone", "ozone_no_interp",
      ...
      "a1c_exm_pct_no_interp", "tmmx", "rmax", "pr", "hmo_mo",
      "entry_age", "entry_year", "followup_year", "followup_year_plus_one")
...
n_unique <- rbindlist(
    mclapply(c(id.vars, cat.vars, num.vars),
           function(var) {
...
fwrite(all_stats, paste0(resultsdir, "merged_1999_2016_data_stats.csv"))
save(all_stats, n_unique, tab, numsum,
     file = paste0(resultsdir, "merged_1999_2016_data_stats.RData"))
